...
class Hyperplane(object):
    ...
    def first_nonzero_index(iterable):
        index = 0
        for index in range(iterable.dimension):
            if not MyDecimal(iterable[index]).is_near_zero():
                return index
            index += 1
...
# p1 = Hyperplane(normal_vector=Vector(['-7.926', '8.625', '-7.212']), constant_term='-7.952')
# p2 = Hyperplane(normal_vector=Vector(['-2.642', '2.875', '-2.404']), constant_term='-2.443')
